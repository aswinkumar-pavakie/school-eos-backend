...
export async function seedTier12b(ctx: SeedContext, t0: Tier0Ids, t1: Tier1Ids, t2: Tier2Ids) {
  ...
  // --- complaint: top up to 49 (17 already committed) + 71 updates on real complaint ids ---
  const existingComplaints = Number((await ctx.query<{ n: string }>(`SELECT count(*) n FROM complaint`)).rows[0]!.n);
  for (let i = existingComplaints; i < 49; i++) {
    await ctx.insertReturningId("complaint", {
      raised_by_person_id: pick([...allStaffPersonIds, ...t2.guardianPersonIds]), category: pick(["FACILITY", "ACADEMIC", "STAFF_CONDUCT"]),
      ...
      severity: pick(["LOW", "NORMAL", "HIGH"]), state: rand() < 0.7 ? "RESOLVED" : rand() < 0.9 ? "IN_PROGRESS" : "ESCALATED",
    });
  }
  const complaintIds = (await ctx.query<{ id: string }>(`SELECT id FROM complaint`)).rows.map((r) => r.id);
  for (let i = 0; i < 71; i++) {
