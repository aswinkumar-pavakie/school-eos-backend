...
export async function seedTier13b(ctx: SeedContext, t0: Tier0Ids, t1: Tier1Ids, t2: Tier2Ids, t3: Tier3Ids) {
  ...
  // --- approval_request: decided_at must be present exactly when the request is decided ---
  const approvalRequestRows: unknown[][] = [];
  for (let i = 0; i < 178; i++) {
    const approved = rand() < 0.85;
    approvalRequestRows.push([pick(["LEAVE_REQUEST", "PURCHASE_REQUEST", "SPORTS_BUDGET_REQUEST"]), "STAFF_LEAVE_REQUEST", crypto.randomUUID(), pick(allStaffPersonIds), JSON.stringify({}), 1, approved ? "APPROVED" : "PENDING", approved ? new Date(2025, 6 + (i % 9), 1 + (i % 27), 11).toISOString() : null]);
  }
  await ctx.insertMany("approval_request", ["request_type", "subject_object_type", "subject_object_id", "requested_by", "payload", "current_step", "state", "decided_at"], approvalRequestRows);
  const approvalReqIds = (await ctx.query<{ id: string; state: string }>(`SELECT id, state FROM approval_request`)).rows;
  await ctx.insertMany("approval_step", ["request_id", "sequence_no", "approver_role_code", "decided_by", "decision", "decided_at"],
    approvalReqIds.map((r, i) => [r.id, 1, "PRINCIPAL", r.state === "APPROVED" ? principalId : null, r.state === "APPROVED" ? "APPROVED" : null, r.state === "APPROVED" ? new Date(2025, 6 + (i % 9), 1 + (i % 27), 11).toISOString() : null]));

  // --- camp_partner (2) + camp (3) ---
  const partnerIds: string[] = [];
  for (const name of ["Aravind Eye Hospital", "Meenakshi Mission Hospital"]) {
    partnerIds.push(await ctx.insertReturningId("camp_partner", { name, partner_type: "HOSPITAL", status: "ACTIVE" }));
  }
  const campDefs = [["Eye Check-up Camp", "VISION"], ["Dental Camp", "DENTAL"], ["Blood Grouping Camp", "BLOOD_GROUP"]] as const;
  for (const [name, type] of campDefs) {
    await ctx.insertReturningId("camp", {
      name, camp_type: type, partner_id: pick(partnerIds), organiser_staff_id: t1.staff.find((s) => !s.isTeaching)!.staffId, academic_year_id: t0.academicYearId,
      venue: "School Auditorium", start_date: "2025-10-15", end_date: "2025-10-15", target_scope_type: "SCHOOL",
  ...
  await ctx.insertMany("community_membership", ["community_id", "student_id", "role_in_community", "joined_on", "status", "parent_consent_at"], memberStudents.map((id) => [communityId, id, "MEMBER", "2025-06-15", "ACTIVE", "2025-06-14T10:00:00Z"]));
  ...
  await ctx.insertMany("document", ["owner_domain", "owner_object_type", "owner_object_id", "category", "doc_type", "object_key", "file_name", "mime_type", "size_bytes", "uploaded_by", "status"],
    Array.from({ length: 5 }, (_, i) => ["ORGANIZATION", "POLICY", "school", "POLICY", "HANDBOOK", `docs/school/handbook-${i}.pdf`, "Student_Handbook.pdf", "application/pdf", 500000, principalId, "ACTIVE"]));

  // --- lms_material (24, real per-subject reference material) ---
  await ctx.insertMany("lms_material", ["subject_id", "medium_id", "title", "material_type", "object_key", "published_at", "status"],
    Object.values(t0.subjectIds).slice(0, 24).map((id) => [id, t0.mediumIds.english, "Reference Material", "PDF", `lms/${id}/reference-material.pdf`, new Date().toISOString(), "ACTIVE"]));
  ...
  await ctx.insertMany("media_post", ["format", "category", "caption", "state", "published_at", "created_by"],
    ["Annual Day Highlights - real photos from our wonderful celebration!", "Sports Day Winners - congratulations to all our champions!"].map((caption) => ["PHOTO_CAROUSEL", "EVENT", caption, "PUBLISHED", new Date().toISOString(), mediaStaffPersonIds[0]]));
  ...
  for (let i = 0; i < 120; i++) inventoryItemRows.push([`Asset ${i + 1}`, pick(invCategoryIds), `AST-${String(i + 1).padStart(4, "0")}`, 1 + Math.floor(rand() * 20), 5, "AVAILABLE"]);
  ...
  await ctx.insertMany("repair_request", ["title", "inventory_item_id", "issue_type", "priority", "description", "status", "requested_on", "requested_by"],
    Array.from({ length: 80 }, () => { const [title, type] = pick([["Broken fan", "ELECTRICAL"], ["Leaking tap", "PLUMBING"], ["Projector fault", "IT_EQUIPMENT"]] as const); return [title, pick(invItemIds).id, type, pick(["LOW", "MEDIUM", "HIGH"]), "Facility issue reported and logged.", rand() < 0.75 ? "COMPLETED" : "REQUESTED", isoDate(new Date()), pick(allStaffPersonIds)]; }));
  ...
  await ctx.insertMany("card_tap_event", ["card_uid", "terminal_id", "terminal_type", "tapped_at", "received_at", "tap_result", "device_idempotency_key", "is_offline_capture", "context_type", "auth_method"],
    Array.from({ length: 400 }, (_, i) => [`CARD-${1000 + i}`, pick(t0.terminalIds), "CANTEEN_POS", new Date().toISOString(), new Date().toISOString(), "ACCEPTED", `tap-${i}`, false, "CANTEEN_SALE", "AES_MUTUAL"]));
  ...
  for (const trip of busRiderTripsRes.rows) {
    ...
    boardingRows.push([trip.id, pick(allStudentIds), stopsRes.rows[0].id, pick(["BOARD", "ALIGHT"]), "CARD_TAP", false, vra?.attendant_id ?? null, new Date().toISOString()]);
  }
  ...
  await ctx.insertMany("camp_follow_up", ["checkup_record_id", "student_id", "action", "status", "completed_on"], abnormalCheckups.map((c) => [c.id, c.student_id, "Referred for specialist consultation", "DONE", "2025-11-05"]));
  ...
  await ctx.insertMany("gate_movement", ["gate_pass_id", "direction", "recorded_at"], gatePassIds.map((g) => [g.id, "OUT", new Date().toISOString()]));
  ...
  await ctx.insertMany("medical_escalation", ["source_type", "source_id", "student_id", "sequence_no", "contacted_person_id", "contacted_at", "outcome", "channel"],
    Array.from({ length: 10 }, () => { const g = pick(t2.guardianPersonIds); return ["VISIT", crypto.randomUUID(), pick(allStudentIds), 1, g, new Date().toISOString(), "Parent informed and arrived.", "PHONE"]; }));
  ...
  for (let i = 0; i < 8; i++) {
    ...
    const id = await ctx.insertReturningId("permission_activity", {
      ...
      title: `Field Trip Permission - Activity ${i + 1}`, permission_type: "TRIP", activity_date: isoDate(new Date(2025, 8 + i, 10)),
      start_time: "09:00", end_time: "16:00", response_deadline: isoDate(new Date(2025, 8 + i, 5)), status: "ACTIVE",
    });
  ...
  for (const act of permActivityIds) {
    for (const studentId of (studentsBySection.get(act.sectionKey) ?? []).slice(0, 40)) {
      permRequestRows.push([act.id, studentId, rand() < 0.85 ? "CONSENTED" : "PENDING"]);
    }
  }
  ...
  await ctx.insertMany("sports_injury_incident", ["student_id", "title", "incident_date", "guardian_informed", "status"],
    Array.from({ length: 2 }, () => [pick(allStudentIds), "Minor sprain during practice", isoDate(new Date()), true, "CLOSED"]));
  ...
  for (let d = 0; d < 30; d++) {
    ...
    for (const s of t1.staff) {
      ...
      staffAttendanceRows.push([s.staffId, eventType, "BIOMETRIC_FACE", date.toISOString(), date.toISOString(), "CONFIRMED"]);
    }
  }
  ...
  await ctx.insertMany("student_trip_status", ["trip_id", "student_id", "status", "boarded_at", "boarded_stop_id", "last_updated_at"],
    boardingEventsRes.rows.map((b) => [b.trip_id, b.student_id, b.direction === "BOARD" ? "BOARDED" : "DROPPED", b.recorded_at, b.route_stop_id, b.recorded_at]));
  ...
  await ctx.insertMany("substitution", ["timetable_slot_id", "sub_date", "original_staff_id", "substitute_staff_id", "reason", "assigned_by", "status"],
    slotRes.rows.map((slot, i) => [slot.id, isoDate(new Date()), approvedLeaveRes.rows[i % approvedLeaveRes.rows.length]?.staff_id ?? t1.staff[0]!.staffId, pick(teachingPersonIds.map((pid) => t1.staff.find((s) => s.personId === pid)!.staffId)), "Original teacher on approved leave.", principalId, "ASSIGNED"]));
  ...
  await ctx.insertMany("terminal_sync_log", ["terminal_id", "sync_type", "blocklist_version", "roster_version", "record_count", "synced_at", "outcome"],
    t0.terminalIds.map((id, i) => [id, ["ROSTER", "BLOCKLIST", "BALANCES", "LIMITS", "MENU", "TRANSACTIONS"][i % 6], 1, 1, 500, new Date().toISOString(), "SUCCESS"]));
