...
export class PersonsService {
  ...
  async get(personId: string) {
    ...
    const roleAssignments =
      await this.roleAssignmentRepo.findAllByPersonId(personId);
  ...
  async create(dto: CreatePersonDto, actorPersonId: string) {
    ...
    if (dto.initialRole.roleCode === 'ADMIN') {
      throw new BadRequestException(
        'Admin accounts cannot be created through this screen.',
      );
    }
    ...
    if (!roleExists) {
      throw new BadRequestException(
        `Unknown role_code: ${dto.initialRole.roleCode}`,
      );
    }
        ...
        const person = await this.personRepo.create(
          {
            ...
            mobile:
              dto.identifierType === 'MOBILE' ? dto.identifierValue : null,
  ...
  async update(personId: string, dto: UpdatePersonDto, actorPersonId: string) {
    ...
    try {
      const updated = await this.personRepo.update(
        personId,
        dto,
        actorPersonId,
      );
  ...
  async deactivate(personId: string, actorPersonId: string): Promise<void> {
    ...
    await this.unitOfWork.run(async (client) => {
      await this.personRepo.updateStatus(
        personId,
        'SUSPENDED',
        actorPersonId,
        client,
  ...
  async uploadPhoto(
    ...
    const updated = await this.personRepo.updatePhoto(
      personId,
      photoObjectKey,
      actorPersonId,
    );
