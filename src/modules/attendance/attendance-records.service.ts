...
export class AttendanceRecordsService {
  ...
  async update(
    ...
    const updated = await this.recordRepo.updateStatus(
      id,
      dto.status,
      dto.reason,
    );
  ...
  /** Read-only lookup for other modules (Hostel's Class Absence Alert) -- never
   * writes to attendance_record/attendance_session. See
   * AttendanceRecordRepository.findAbsentStudentIdsForDate. */
  async findAbsentStudentIdsForDate(studentIds: string[], date: string) {
    return this.recordRepo.findAbsentStudentIdsForDate(studentIds, date);
  }

  async getAttendanceSummaryForStudent(studentId: string) {
    const { presentCount, totalCount } =
      await this.recordRepo.getAttendanceSummaryForStudent(studentId);
    return {
      presentCount,
      totalCount,
      percentage:
