...
} from '../attendance/repositories/attendance-record.repository';
import {
  AttendanceSessionRepository,
  AttendanceSessionRow,
} from '../attendance/repositories/attendance-session.repository';
...
export class FacultyAttendanceService {
  ...
  async assertAdvisor(personId: string, sectionId: string): Promise<void> {
    ...
    if (!isAdvisor) {
      throw new ForbiddenException(
        'You are only able to mark attendance for a class you are the class advisor of.',
      );
    }
  }
  ...
  async getOrCreateRoster(
    ...
    return this.unitOfWork.run(async (client) => {
      let session = await this.sessionRepo.findBySectionAndDate(
        sectionId,
        date,
        client,
      );
  ...
  private async assertRecordInSection(
    recordId: string,
    sectionId: string,
    client?: Queryable,
  ) {
  ...
  async markRecord(
    ...
    if (session.isLocked) {
      return this.recordsService.correct(
        recordId,
        {
          newStatus: dto.status,
          reason: dto.reason ?? 'Corrected by class advisor',
        },
        personId,
      );
    }
  ...
  async markAllPresent(personId: string, sectionId: string, date: string) {
    await this.assertAdvisor(personId, sectionId);
    const { session, records } = await this.getOrCreateRoster(
      personId,
      sectionId,
      date,
    );
    for (const record of records) {
      ...
      if (session.isLocked) {
        await this.recordsService.correct(
          record.id,
  ...
  async getHistory(
    ...
    await this.assertAdvisor(personId, sectionId);
    const { rows: sessions } = await this.sessionRepo.findMany({
      sectionId,
      dateFrom: monthStart,
      dateTo: monthEnd,
      limit: 62,
  ...
  async ensureSessionAndMarkStatus(
    ...
    const record = await this.recordRepo.findBySessionAndStudent(
      session.id,
      params.studentId,
      client,
    );
    if (!record) {
      await this.recordRepo.createOne(
        session.id,
        params.studentId,
        params.status,
        params.reason,
      ...
      await this.correctionRepo.create(
        {
          ...
          correctedBy: params.actorPersonId,
        },
        client,
      );
    } else {
      await this.recordRepo.updateStatus(
        record.id,
        params.status,
        params.reason,
        client,
