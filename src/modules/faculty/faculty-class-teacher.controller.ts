...
export class FacultyClassTeacherController {
  ...
  async dashboard(@Param('sectionId', ParseUUIDPipe) sectionId: string, @CurrentActor() actor: AuthenticatedUser) {
  ...
  async searchStudents(
    ...
    return { data: await this.service.searchStudents(actor.personId, sectionId, q ?? '') };
  }

  @Get('sections/:sectionId/duties')
  async listDuties(@Param('sectionId', ParseUUIDPipe) sectionId: string, @CurrentActor() actor: AuthenticatedUser) {
  ...
  async createDuty(
    ...
    return { data: await this.service.createDuty(actor.personId, sectionId, dto) };
  }

  @Patch('duties/:id')
  async updateDuty(@Param('id', ParseUUIDPipe) id: string, @Body() dto: UpdateStudentDutyDto, @CurrentActor() actor: AuthenticatedUser) {
    return { data: await this.service.updateDuty(actor.personId, id, dto) };
  }

  @Delete('duties/:id')
  @HttpCode(HttpStatus.NO_CONTENT)
  async removeDuty(@Param('id', ParseUUIDPipe) id: string, @CurrentActor() actor: AuthenticatedUser) {
