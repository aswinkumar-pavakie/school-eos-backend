...
export class FacultySubjectRecordsService {
  ...
  async getRecords(personId: string, subjectOfferingId: string) {
    ...
    const { rows: offeringRows } = await this.postgres.query(
      `SELECT section_id FROM subject_offering WHERE id = $1`,
      [subjectOfferingId],
    );
    const sectionId = offeringRows[0]?.section_id;
    if (!sectionId)
      throw new ForbiddenException('This subject offering no longer exists.');

    const rows = await this.marksRepo.findPublishedMarksForOffering(
      sectionId,
      subjectOfferingId,
    );
    ...
    for (const entry of byStudent.values()) {
      ...
      const total =
        Math.round(
          scored.reduce((sum: number, e: any) => sum + e.marksObtained, 0) *
            100,
        ) / 100;
      ...
      const [attendance, guardian] = await Promise.all([
        this.attendanceRecordsService.getAttendanceSummaryForStudent(
          entry.studentId,
        ),
  ...
  private async getPrimaryGuardianPhone(
    studentId: string,
  ): Promise<string | null> {
