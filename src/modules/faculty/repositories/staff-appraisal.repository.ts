...
export class StaffAppraisalRepository {
  ...
  async findByStaffId(staffId: string, executor: Queryable = this.postgres): Promise<StaffAppraisalRow[]> {
    const { rows } = await executor.query(`SELECT ${COLUMNS} ${FROM} WHERE sa.staff_id = $1 ORDER BY sa.created_at DESC`, [staffId]);
    return rows.map(mapRow);
  }

  async findById(id: string, executor: Queryable = this.postgres): Promise<StaffAppraisalRow | null> {
    const { rows } = await executor.query(`SELECT ${COLUMNS} ${FROM} WHERE sa.id = $1`, [id]);
    return rows.length ? mapRow(rows[0]) : null;
  }

  async create(
    input: { staffId: string; cycle: string; selfAssessment: string; attachmentObjectKey: string | null; attachmentFileName: string | null },
    executor: Queryable,
  ): Promise<string> {
    const { rows } = await executor.query(
      `INSERT INTO staff_appraisal (staff_id, cycle, self_assessment, attachment_object_key, attachment_file_name)
       VALUES ($1, $2, $3, $4, $5) RETURNING id`,
      [input.staffId, input.cycle, input.selfAssessment, input.attachmentObjectKey, input.attachmentFileName],
    );
    return rows[0].id;
  }
  ...
  async markReviewed(id: string, reviewedBy: string, executor: Queryable): Promise<void> {
