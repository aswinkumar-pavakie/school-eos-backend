...
export class StaffHrRequestRepository {
  ...
  async findByStaffId(
    ...
    const { rows } = await executor.query(
      `SELECT ${COLUMNS} FROM staff_hr_request WHERE staff_id = $1 ORDER BY created_at DESC`,
      [staffId],
    );
  ...
  async findById(
    id: string,
    executor: Queryable = this.postgres,
  ): Promise<StaffHrRequestRow | null> {
    const { rows } = await executor.query(
      `SELECT ${COLUMNS} FROM staff_hr_request WHERE id = $1`,
      [id],
    );
  ...
  async linkApprovalRequest(
    ...
    await executor.query(
      `UPDATE staff_hr_request SET approval_request_id = $2, updated_at = now() WHERE id = $1`,
