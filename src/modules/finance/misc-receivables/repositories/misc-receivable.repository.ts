...
export class MiscReceivableRepository {
  ...
  async findMany(filter: ReceivableFilter, executor: Queryable = this.postgres): Promise<{ rows: MiscReceivableRow[]; total: number }> {
  ...
  async findById(id: string, executor: Queryable = this.postgres): Promise<MiscReceivableRow | null> {
    const { rows } = await executor.query<MiscReceivableRow>(`SELECT ${COLUMNS} FROM ${FROM} WHERE r.id = $1`, [id]);
    return rows[0] ?? null;
  }

  async findByIdForUpdate(id: string, executor: Queryable): Promise<MiscReceivableRow | null> {
    const { rows } = await executor.query<MiscReceivableRow>(
  ...
  async create(input: CreateReceivableInput, executor: Queryable = this.postgres): Promise<MiscReceivableRow> {
    const { rows } = await executor.query<{ id: string }>(
      `INSERT INTO finance_misc_receivable (source_module, source_reference_id, person_id, description, amount_paise)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id`,
      [input.sourceModule, input.sourceReferenceId, input.personId, input.description, input.amountPaise],
    );
  ...
  async recordPayment(
    receivableId: string,
    input: { amountPaise: number | string; mode: string; collectedBy: string; idempotencyKey: string },
    executor: Queryable,
  ): Promise<void> {
    await executor.query(
      `INSERT INTO finance_misc_receivable_payment (receivable_id, amount_paise, mode, collected_by, idempotency_key)
       VALUES ($1, $2, $3, $4, $5)`,
      [receivableId, input.amountPaise, input.mode, input.collectedBy, input.idempotencyKey],
    );
  }

  async applyPayment(receivableId: string, amountPaise: number | string, executor: Queryable): Promise<MiscReceivableRow> {
    await executor.query(
