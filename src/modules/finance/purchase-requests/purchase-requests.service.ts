...
export class PurchaseRequestsService {
  ...
  async create(
    input: {
      ...
      neededBy?: string;
      departmentId?: string;
      // Set only by Sports' own indent controller — links this request to a
      // specific equipment row so approved+delivered stock can be auto-applied
      // (see allotOrder below). Every other caller leaves this undefined.
      equipmentId?: string;
    },
   ...
   *
   * If the originating purchase_request is linked to a specific equipment row
   * (Sports' own indent flow — see SportsEquipmentIndentsController), the
   * allotted quantity is also applied to that equipment's own quantity_total/
   * quantity_available, in the same transaction — a Sports restock finishing
   * fulfillment shows up in the sport's inventory immediately, with no manual
   * re-entry. Every other caller (Admin/Media Room) leaves equipmentId null and
    ...
    return this.unitOfWork.run(async (client) => {
      ...
      if (request?.equipmentId) {
        await client.query(
          ...
          [request.equipmentId, input.quantity],
        );
      }
      ...
      await this.audit.record(
        {
          ...
          objectId: id,
          outcome: 'SUCCESS',
          afterData: { ...input, equipmentId: request?.equipmentId ?? null },
        },
        client,
      );
      return updated;
