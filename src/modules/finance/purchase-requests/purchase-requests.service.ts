...
import { PurchaseOrderRepository, PurchaseOrderRow, PurchaseOrderStage, PurchaseOrderSummary } from './repositories/purchase-order.repository';
import { PurchaseRequestRepository, PurchaseRequestRow, PurchaseRequestSummary } from './repositories/purchase-request.repository';
...
export class PurchaseRequestsService {
      ...
      const approvalRequest = await this.approvalsService.createRequest(
        {
          ...
          payload: { itemName: input.itemName, quantity: input.quantity, vendorName: input.vendorName },
        },
        client,
      );
      await this.requestRepo.linkApprovalRequest(request.id, approvalRequest.id, client);
  ...
  async list(
    filter: { state?: string; requestType?: string; departmentId?: string; search?: string },
    page: PageQuery,
    actor: AuthenticatedUser,
  ) {
  ...
  async summary(requestType: 'GOODS' | 'SERVICE'): Promise<PurchaseRequestSummary> {
    return this.requestRepo.summary(requestType);
  }

  async getById(id: string): Promise<{ request: PurchaseRequestRow; order: PurchaseOrderRow | null }> {
  ...
  async listOrders(filter: { stage?: string; requestType?: string; search?: string }, page: PageQuery) {
    return this.orderRepo.list(filter, page);
  }

  /** Real, DB-computed KPI numbers for the POP/SOP tracking board. */
  async ordersSummary(requestType: 'GOODS' | 'SERVICE'): Promise<PurchaseOrderSummary> {
    return this.orderRepo.summary(requestType);
  }

  async getOrderById(id: string): Promise<{ order: PurchaseOrderRow; events: Awaited<ReturnType<PurchaseOrderRepository['listEvents']>> }> {
  ...
  async updateOrderStage(
    id: string,
    input: { stage: PurchaseOrderStage; quantityDelivered?: number; note?: string },
    actor: AuthenticatedUser,
  ): Promise<PurchaseOrderRow> {
    return this.unitOfWork.run(async (client) => {
      const order = await this.orderRepo.findByIdForUpdate(id, client);
      if (!order) throw new NotFoundException('Purchase order not found');
      if (input.quantityDelivered !== undefined && input.quantityDelivered > order.quantityOrdered) {
        throw new ConflictException('Quantity delivered cannot exceed quantity ordered');
      }
  ...
  async allotOrder(id: string, input: { quantity: number; note?: string }, actor: AuthenticatedUser): Promise<PurchaseOrderRow> {
    return this.unitOfWork.run(async (client) => {
      const order = await this.orderRepo.findByIdForUpdate(id, client);
      if (!order) throw new NotFoundException('Purchase order not found');
      if (order.quantityAllotted + input.quantity > order.quantityDelivered) {
        throw new ConflictException('Allotment cannot exceed what has been delivered');
      }
      const updated = await this.orderRepo.addAllotted(id, input.quantity, client);
      await this.audit.record(
        {
          actorPersonId: actor.personId,
          actorRoleCode: 'FINANCE',
