...
export interface PurchaseRequestRow {
  ...
  // Nullable — only set by callers (Sports) that need approved+delivered stock
  // auto-applied to one specific equipment row; Admin/Media Room requests leave
  // this null and are otherwise unaffected (see PurchaseRequestsService.allotOrder).
        ...
        const { rows } = await executor.query(
          `INSERT INTO purchase_request
             (reference_no, request_type, item_name, description, quantity, vendor_name,
              estimated_amount_paise, needed_by, department_id, requested_by, equipment_id, state)
