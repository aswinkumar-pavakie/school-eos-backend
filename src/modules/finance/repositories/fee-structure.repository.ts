...
export class FeeStructureRepository {
  ...
  async create(
    ...
    const { rows } = await executor.query<FeeStructureRow>(
      `INSERT INTO fee_structure (academic_year_id, grade_id, medium_id, category, total_paise, state)
       ...
       RETURNING ${COLUMNS}`,
      [input.academicYearId, input.gradeId, input.mediumId ?? null, input.category ?? null],
    );
    return rows[0];
  }
  ...
  async update(
    ...
    const { rows } = await executor.query<FeeStructureRow>(
      `UPDATE fee_structure SET
       ...
       WHERE id = $1
       RETURNING ${COLUMNS}`,
      [id, input.academicYearId ?? null, input.gradeId ?? null, input.mediumId ?? null, input.category ?? null],
    );
    return rows[0] ?? null;
  }
  ...
  async recomputeTotal(id: string, executor: Queryable): Promise<FeeStructureRow | null> {
    const { rows } = await executor.query<FeeStructureRow>(
