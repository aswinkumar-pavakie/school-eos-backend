...
export class HostelWardenGatePassController {
  ...
  async get(
    ...
    return {
      data: await this.service.getOuting(actor.personId, 'GATE_PASS', id),
    };
  }
  ...
  async approve(
    @Param('id', ParseUUIDPipe) id: string,
    @Body() dto: ApproveOutingRequestDto,
    @CurrentActor() actor: AuthenticatedUser,
  ) {
    return {
      data: await this.service.decideOuting(
        actor.personId,
        'GATE_PASS',
        id,
        'APPROVED',
...
export class HostelWardenEmergencyExitController {
  ...
  async list(@CurrentActor() actor: AuthenticatedUser) {
    return {
      data: await this.service.listOuting(actor.personId, 'EMERGENCY_EXIT'),
    };
  }
  ...
  async get(
    ...
    return {
      data: await this.service.getOuting(actor.personId, 'EMERGENCY_EXIT', id),
    };
  }
...
export class HostelWardenCallRequestsController {
  ...
  async approve(
    ...
    return {
      data: await this.service.decideCall(actor.personId, id, {
        status: 'APPROVED',
        approvedFrom: dto.approvedFrom,
