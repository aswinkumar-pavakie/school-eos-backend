...
export class CallRequestRepository {
  ...
  async findForHostels(
    hostelIds: string[],
    executor: Queryable = this.postgres,
  ): Promise<CallRequestRow[]> {
    if (hostelIds.length === 0) return [];
    const { rows } = await executor.query(
      `SELECT ${COLUMNS} FROM ${FROM} WHERE c.hostel_id = ANY($1) ORDER BY c.created_at DESC`,
      [hostelIds],
    );
  ...
  async findByStudent(
    ...
    const { rows } = await executor.query(
      `SELECT ${COLUMNS} FROM ${FROM} WHERE c.student_id = $1 ORDER BY c.created_at DESC`,
      [studentId],
    );
    return rows.map(mapRow);
  }
  ...
  async findByParent(
    ...
    const { rows } = await executor.query(
      `SELECT ${COLUMNS} FROM ${FROM} WHERE c.parent_person_id = $1 ORDER BY c.created_at DESC`,
      [parentPersonId],
    );
    return rows.map(mapRow);
  }
