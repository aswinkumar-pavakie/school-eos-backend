...
} from './repositories/gateway-order.repository';
import {
  FeeLineRow,
  ParentFeeRepository,
} from './repositories/parent-fee.repository';
...
function splitAllocation(
  ...
  for (const id of orderedIds) {
    ...
    if (!line) continue;
    const balance =
      BigInt(line.amountPaise) +
      BigInt(line.lateFeePaise) -
      BigInt(line.paidPaise);
...
export class ParentFeesService {
  ...
  async listTerms(actor: AuthenticatedUser, studentId: string) {
    ...
    return terms.map((t) => ({
      ...t,
      label: `Term ${t.instalmentNo} · ${t.academicYearName}`,
    }));
  }
  ...
  async getFeeSummary(
    ...
    const lines = await this.parentFeeRepo.listLinesForTerm(
      studentId,
      academicYearId,
      instalmentNo,
    );
    ...
    return {
      ...
      lines: lines.map((l) => ({
        ...
        outstandingPaise: (
          BigInt(l.amountPaise) +
          BigInt(l.lateFeePaise) -
          BigInt(l.paidPaise)
        ).toString(),
        dueDate: l.dueDate,
        state: l.state,
  ...
  async createRazorpayOrder(
    ...
    const lines = await this.parentFeeRepo.findManyForStudent(
      studentId,
      dto.feeDemandIds,
    );
    ...
    const order = await this.razorpayService.createOrder({
      ...
      notes: {
        ...
        instalmentNo: String(dto.instalmentNo),
      },
    });
  ...
  async handleRazorpayWebhook(body: any): Promise<{ acknowledged: true }> {
    ...
    const gatewayOrder = await this.gatewayOrderRepo.findByGatewayOrderId(
      'RAZORPAY',
      entity.order_id,
    );
    if (!gatewayOrder) {
    ...
    if (status === 'CONFIRMED' && result.matched) {
      try {
        await this.paymentsService.allocate(
          gatewayOrder.paymentId,
          gatewayOrder.allocations,
        );
      } catch (err) {
